...
function booking_callback(){
 ...
 $booked_service_name = $wpdb->get_var($wpdb->prepare('SELECT name FROM ' . $table_name . ' WHERE id = %d', intval($_REQUEST['bservice'])));
 ...
 $emp_nam = $wpdb->get_var($wpdb->prepare('SELECT emp_name  FROM ' . $table_name . ' WHERE id = %d', esc_attr($_REQUEST['empnam'])));
 $emp_email = $wpdb->get_var($wpdb->prepare('SELECT email  FROM ' . $table_name . ' WHERE id = %d', esc_attr($_REQUEST['empnam'])));
 ...
 $minformat = $wpdb->get_var($wpdb->prepare('SELECT minuteformat  FROM ' . $table_name));
 if($minformat==0)
 {
		if($etmins>30)
		{
 ...
 else
 {
    ...
    $existing_client = $wpdb->get_var($wpdb->prepare('SELECT count(email) FROM ' . $table_name . ' WHERE email = %s', $client_email));		
	$curdate =  $wpdb->get_var("SELECT STR_TO_DATE('". intval($_REQUEST['byear']) . "," . intval($_REQUEST['bmonth']) . "," . intval($_REQUEST['bday']) . "','%Y,%m,%d')");
	if($existing_client>0)
	{
			$existing_client_id = $wpdb->get_var($wpdb->prepare('SELECT id FROM ' . $table_name . ' WHERE email = %s', $client_email));		
			$add1 = $wpdb->get_var($wpdb->prepare('SELECT addressLine1 FROM ' . $table_name . ' WHERE email = %s', $client_email));		
		...
		$empcolor=$wpdb->get_var($wpdb->prepare("SELECT emp_color FROM ". $wpdb->prefix . sm_employees. ' where id= %d', intval($_REQUEST['empnam'])));
		$countt=$wpdb->get_var($wpdb->prepare("SELECT count(id) FROM ". $wpdb->prefix . sm_bookings. ' where StartTime =%s and EndTime  = %s and emp_id = %d and service_id  = %d and status = Approval Pending', $sttime, $etime, $empid, intval($_REQUEST['bservice'])));
	    if($countt==0)
		{
			$wpdb->query
	            (
	    ...
	    $approve = "<a href=\"".admin_url()."admin-ajax.php?action=confirm&actionappbok=approve&id=".$b_id[0]."\">Approve Booking</a>";
	    $msg_5 = str_replace("[approve]", $approve, $msg_4);
	    $disapprove = "<a href=\"".admin_url()."admin-ajax.php?action=confirm_disapprove&booking=".$b_id[0]."\">Disapprove Booking</a>";
	...
	else
	{
		...
		$c_id = $wpdb->insert_id;
        $new_client_id = $wpdb->get_var($wpdb->prepare('SELECT id FROM ' . $table_name . ' WHERE email = %s', $client_email));
        $table_name = $wpdb->prefix . "sm_bookings";
		$empid=intval($_REQUEST['empnam']);	
		$empcolor=$wpdb->get_var($wpdb->prepare("SELECT emp_color FROM ". $wpdb->prefix . sm_employees. ' where id= %d', intval($_REQUEST['empnam'])));
        ...
        $table_name = $wpdb->prefix . "sm_emails";
        $msg_stored = $wpdb->get_var($wpdb->prepare('SELECT content  FROM ' . $table_name . ' WHERE type = "admin"'));
		$msg_subject = $wpdb->get_var($wpdb->prepare('SELECT subject FROM ' . $table_name . ' WHERE type = "admin"'));
        $msg_1 = str_replace("[client_name]", esc_attr($_REQUEST['bname']), $msg_stored);
        $msg_2 = str_replace("[service_name]", $booked_service_name, $msg_1);
        $msg_3 = str_replace("[booked_time]", esc_html($_REQUEST['btime']), $msg_2);
        $msg_4 = str_replace("[booked_date]", $date, $msg_3);
		...
		wp_mail($admin_email,$msg_subject,$msg_8,$headers);	
	}
	die;
}
?>