...
function booking() 
{
		...
		jQuery('#online_booking').load('<?php echo admin_url('admin-ajax.php'); ?>' + "?action=online_booking",function(){jQuery('#loading').remove();jQuery('.maincontainer1').css('display', 'block');});
				...
				<span class="iconsweet">b</span>
					<h5>Bookings</h5>
							...
							<form id="expo" method="post" action="<?php echo $uri;?>exportbooking.php" >
								...
								<a href="#" onClick="booking();" style="width:100px; margin-left:20px;font-weight: normal !important;text-shadow: none !important;" class="redishBtn button_small">Book a Service</a>
								</div>
								</li>
								</ul>
...
<a id="hiddenclicker" href="#Window" class="colorbox" >Hidden Clicker</a>
</div>
<div style="display:none;">
<div style="width:600px;" id="Window"></div>
</div>
</div>
	...
	function loademp()
	{
		...
		jQuery(document).ready(function()
		{
			...
			({
				...
				url: '<?php echo admin_url('admin-ajax.php?action=getworkinghours'); ?>',
				success: function(data) 
				{
					jQuery("#divhid").html(data);
					var view="month";
					var DATA_FEED_URL = '<?php echo admin_url('admin-ajax.php'); ?>';
					var op = 
					{
						view: view,
						theme:3,
						...
						autoload:true,
						url: DATA_FEED_URL + '?action=datafeed&method=list&status='+stat,
						quickAddUrl: DATA_FEED_URL + "?action=datafeed&method=add",
						quickUpdateUrl: DATA_FEED_URL + "?action=datafeed&method=update",
						quickDeleteUrl: DATA_FEED_URL + "?action=datafeed&method=remove"
					};
					...
					function Edit(data)
					{
						var eurl='<?php echo admin_url('admin-ajax.php'); ?>' + "?action=edit_booking&id={0}&start={2}&end={3}&isallday={4}&title={1}&empid={10}&empdis={11}&sernm={1}";
						if(data)
						{
								...
								{
									jQuery('#Window').empty();
									jQuery('#Window').load(url,function(){jQuery.colorbox({inline: true, href: '#Window'});});
