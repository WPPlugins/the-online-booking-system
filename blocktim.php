...
{     global $wpdb;
		$dattimeee = $wpdb->get_var(
			$wpdb->prepare(
				'SELECT count(day) FROM ' . $wpdb->prefix . sm_block_date . ' WHERE day = %d AND month = %d AND year = %d AND emp_id = %d',
...
?>
			...
			<select id="addblocktime" multiple='multiple' style="width:100px;height:65px;margin-top:10px;margin-left:60px;" >
										...
										$blktime = $wpdb->get_results(
											$wpdb->prepare(
												'SELECT block_time FROM ' . $wpdb->prefix . sm_block_time . ' WHERE  day = %d AND month = %d AND year = %d AND emp_id = %d order by block_time',
												$day,
												$month,
												$year,
												$val
