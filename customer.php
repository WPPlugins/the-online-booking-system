...
else if($_REQUEST['method'] == 'searchcustomer')
{
			...
			if($vl == "")
			{
				...
				$disp1 = $wpdb -> get_results("SELECT id, name, lastname, email, mobile, telephone, addressLine1, addressLine2, city,postalcode,country,comments FROM " . $wpdb -> prefix . sm_clients . " WHERE name LIKE 'a%' order by name ASC");
			}
			else
			{
				$query ="";
				$disp1 = $wpdb -> get_results($wpdb->prepare("SELECT id, name, lastname, email, mobile, telephone, addressLine1, addressLine2, city,postalcode,country,comments FROM " . $wpdb -> prefix . sm_clients . " WHERE name LIKE '%%%s%%' or lastname LIKE '%%%s%%' or email LIKE '%%%s%%' or mobile LIKE '%%s' or telephone LIKE '%%%s%%' or city LIKE '%%%s%%' or postalcode LIKE '%%%s%%' or country LIKE '%%%s%%'  order by name ASC",
				$vl,
				$vl,
				$vl,
				$vl,
...
else if($_REQUEST['method'] == 'sortingchar')
 {
			...
			$disp = $wpdb -> get_results($wpdb->prepare("SELECT id, name, lastname, email, mobile, telephone, addressLine1, addressLine2, city,postalcode,country,comments FROM " . $wpdb -> prefix . sm_clients . " WHERE name LIKE %s or lastname LIKE %s order by name ASC",
				$ch.'%',
				$ch.'%'
			));
			$count = count($disp);
...
<?php	
	...
	{
			global $wpdb;
			$comments = $wpdb->get_var($wpdb->prepare('SELECT note FROM ' . $wpdb->prefix . sm_bookings . ' WHERE id = %d', $_REQUEST['custoId']));
			?>
                                                   ...
                                                   if(jQuery("#customeremail1").hasClass('in_error'))
                                                   {
													...
													function updatecustomer()
													{
														...
														function bookbackend(h)
														{
															...
															jQuery('#booking_backend').load('<?php echo admin_url('admin-ajax.php'); ?>' + "?action=backendbooking&custId="+hid,function(){jQuery('#loading').remove();jQuery('.maincontainer1').css('display', 'block');});
						...
						<table class="activity_datatable" id="searchcustomer" width="100%" border="0" cellspacing="0" cellpadding="8">
									<tbody>
										...
										<ul class="customer_list_content" id="customerListContainer" onscroll="getMoreCustomersByScroll();" >
											...
											$alpha  = "a";
											$query ="";
											$disp = $wpdb -> get_results("SELECT id, name, lastname, email, mobile, telephone, addressLine1, addressLine2, city,postalcode,country,comments FROM " . $wpdb -> prefix . sm_clients . " WHERE name LIKE 'a%' order by name ASC");
