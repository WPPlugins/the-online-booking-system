<?php
function front_bookinglink_callback(){
global $wpdb;
$url = plugins_url('', __FILE__) . "/"; 

if(isset($_REQUEST['method']))
{
	if($_REQUEST['method'] == "GetEmployees")
	{
...
<script>jQuery("select, input:checkbox, input:radio").uniform(); </script>
		<input type="hidden" id="seridd" value="<?php echo sanitize_text_field($_REQUEST['serviceId']);?>"/>
		...
		<tbody>
			<tr >
				...
				$count= $wpdb->get_var($wpdb->prepare('SELECT count('. $wpdb->prefix . sm_employees . '.id) FROM ' . $wpdb->prefix . sm_employees .'  join '. $wpdb->prefix .sm_allocate_serv .' on ' . $wpdb->prefix . sm_employees . '.id=' .$wpdb->prefix .sm_allocate_serv .'.emp_id where  ' .$wpdb->prefix .sm_allocate_serv . '.serv_id= %d and ' . $wpdb->prefix . sm_employees . '.status = "Active"',
					$sidd
				));
				for($i=0;$i<$count;$i++)
				{
...
<?php
	...
	else if($_REQUEST['method'] == "checkemaill")
	{
		...
		{
			$lname  =  $wpdb->get_var($wpdb->prepare('SELECT lastname  FROM ' . $wpdb->prefix . sm_clients . ' where email = %s', $email));
			$mobile  =  $wpdb->get_var($wpdb->prepare('SELECT mobile  FROM ' . $wpdb->prefix . sm_clients . ' where email = %s', $email));
			$telephone  =  $wpdb->get_var($wpdb->prepare('SELECT telephone  FROM ' . $wpdb->prefix . sm_clients . ' where email = %s', $email));
			$addressLine1  =  $wpdb->get_var($wpdb->prepare('SELECT addressLine1  FROM ' . $wpdb->prefix . sm_clients . ' where email = %s', $email));
			$addressLine2  =  $wpdb->get_var($wpdb->prepare('SELECT addressLine2  FROM ' . $wpdb->prefix . sm_clients . ' where email = %s', $email));
			$city  =  $wpdb->get_var($wpdb->prepare('SELECT city  FROM ' . $wpdb->prefix . sm_clients . ' where email = %s', $email));
			$postalcode  =  $wpdb->get_var($wpdb->prepare('SELECT postalcode  FROM ' . $wpdb->prefix . sm_clients . ' where email = %s', $email));
			$country  =  $wpdb->get_var($wpdb->prepare('SELECT country  FROM ' . $wpdb->prefix . sm_clients . ' where email = %s', $email));
			if($lname=="")
			{
				$lnam = "lname1=yes";
			}
...
$msg1  =  $wpdb->get_var($wpdb->prepare('SELECT translate  FROM ' . $wpdb->prefix . sm_translation . ' where id = 17'));
$msg2  =  $wpdb->get_var($wpdb->prepare('SELECT translate  FROM ' . $wpdb->prefix . sm_translation . ' where id = 18'));
?>	
...
function nextButton1()
{
	...
	if(value != undefined || value != null)
	{
		...
		jQuery.ajax({
					type: "POST",
					data: "serviceId="+value+"&method=GetEmployees",
					url: '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=front_bookinglink",
...
function nextButton2()
{
	...
	if(value != undefined || value != null)
	{
		...
		<?php 
				$minformat  =  $wpdb->get_var($wpdb->prepare('SELECT minuteformat  FROM ' . $wpdb->prefix . sm_settings . ' where id = 1'));
		?>
				var minformat = "<?php echo $minformat; ?>";
				if(minformat==1)
				{
					path= '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=calendar";
				}
				else
				{
					path= '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=calendar30";
				}
	...
	if(document.getElementById('hitime'))
	{
		if(document.getElementById('hitime').value != "")
		{
			...
			({
				...
				url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=servicehours",
				success : function(data) 
				{
		...
		if(jQuery("#reqemaill").val()==1)
		{
			...
			jQuery.ajax
					({
							...
							url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=online_booking",
							success: function(data) 
							{
								if(data=="newcustomer")
								{
					...
					var reg = /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;
						...
						else
						{
							...
							({
								type: "POST",
								data: "email="+address+"&method=checkemaill",
								url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=back_front",
								success: function(data) 
								{
									if(data=="newcustomer")
									{
							...
							if(flag == true && flag1 == true && flag2 == true && flag3 == true && flag4 == true && flag5 == true && flag6 == true)
							{
									...
									({
									type: "POST",
									data: "email="+address+"&method=dataemaill",
									url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=back_front",
									success: function(data) 
									{
										if(jQuery("#emailcustomer").hasClass('in_error'))
		...
		if(value1==1)
		{
			...
			var booked_time= hrsv+":"+mnsv;
				jQuery.ajax({
							...
							url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=booking",
							success: function(data) 
							{
		...
		else
		{
			...
			if(document.getElementById('ext_cntry'))
			{
				...
				jQuery.ajax({
							type: "POST",
							data: data,
							url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=booking_updation",
							success: function(data) 
							{
		...
		<div class="superContainer1">
			<div class="maincontainer1" style="display:none;">
				...
				<?php $table_name = $wpdb -> prefix . "sm_settings";
					   $booked_note = $wpdb -> get_var($wpdb->prepare('SELECT book_header FROM ' . $table_name));
					?>
					<?php echo $booked_note;?>
					</div>
					<?php
								
								$thanks = $wpdb->get_var($wpdb->prepare('SELECT thanks from ' . $wpdb->prefix . sm_settings . ' where id=1'));
								$trans = $wpdb->get_results
								(
										$wpdb->prepare
										(
						...
						<div class="menus">
							<div id="droplinetabs1" class="droplinetabs1">
								...
								<ul class="crBreadcrumbs">
									...
									<div class="widget-wp-obs" style="margin-top:5px;margin-bottom:0px;max-height: 450px;overflow-y:scroll;">
									...
									<table class="activity_datatable" width="100%" border="0" cellspacing="0" cellpadding="8">
										<tbody>
											...
											<?php
														...
														$count =$wpdb->get_var($wpdb->prepare('SELECT count(' . $wpdb->prefix . sm_services .'.id) FROM ' . $wpdb->prefix . sm_services .' join '. $wpdb->prefix .sm_services_time .' on ' . $wpdb->prefix . sm_services . '.id=' .$wpdb->prefix .sm_services_time .'.service_id order by rank ASC'));
														$service_currency = $wpdb->get_var($wpdb->prepare('SELECT currency FROM ' . $wpdb->prefix . sm_currency));
														$cur_icon =  $wpdb->get_var($wpdb->prepare('SELECT currency_sign  FROM ' . $wpdb->prefix . sm_currency . ' where currency_used = "1"'));
														for( $i = 0; $i < $count; $i++)
														{
														$idd=$hour[$i]->id;
														$nam=$hour[$i]->name;
													...
													<?php if($hr==0 && $min!=0)
{
					...
					<div class="one_wrap fl_left" id="step4" style="display:none">
						...
						<ul class="form_fields_container">
								<?php $stt = $wpdb->get_var($wpdb->prepare('SELECT fbradio FROM ' . $wpdb->prefix . sm_settings));
											if($stt==1)
											{
												$api = $wpdb->get_row
												(
							...
							<div class="widget-wp-obs_body" id="Excistingcustomer" style="display:none;">
							 ...
							 <?php }
									...
									<li id="ext_cntry"style="background:none;padding-top:0px;border:none;display:none;">
										...
										<?php
										$sel_country1 = $wpdb -> get_var($wpdb->prepare('SELECT name  FROM ' . $wpdb -> prefix . sm_cuntry . ' where used = 1'));
