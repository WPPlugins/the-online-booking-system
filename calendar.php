...
function draw_calendar($month, $year, $val, $cservice) {
	...
	for ($list_day = 1; $list_day <= $days_in_month; $list_day++) :
		...
		else if ($year < $curr_year) 
		{
			...
			/* add in the day number */
			$sel = "style=\"color: #DEDEDE; text-decoration: line-through;\"";
			$calendar .= '<div onclick="" id="day_number_' . $list_day . '" class="day-number-front" ' . $sel . '>' . $list_day . '</div>';
		} 
		...
		for ($hour = $day_time -> start_hour; $hour <= $day_time -> end_hour; $hour++) 
		{
			for ($minute = 0; $minute <= 45; $minute += 15) 
			{
				for ($matched = 0; $matched <= count($booked); $matched++) 
				{
										...
										for ($j = 0; $j < $gap_interval; $j++) {
											...
											else 
											{
												...
												$booking = $wpdb -> get_var($wpdb->prepare(
													'SELECT count(service_id) FROM ' . $wpdb -> prefix . sm_bookings . ' WHERE day = %d AND hour = %d AND emp_id = %d AND minute = %d AND month = %d AND year = %d AND status != "Disapproved" order by hour,minute ASC',
												...
												$blktime = $wpdb -> get_var($wpdb->prepare(
													'SELECT block_time FROM ' . $wpdb -> prefix . sm_block_time . ' WHERE  day = %d AND month = %d AND year = %d AND block_time = %d AND emp_id = %d',
													$day,
													$month,
													$year,
													$blktm,
												...
												if ($blktime != "") 
												{
													echo "<a class=\"hourschoice_res1 \" rel=\"1\"  href='#'>" . ($hour < 10 ? "0" . $hour : $hour) . ":" . ($minute < 15 ? "0" . $minute : $minute) . "</a>";
												}
												else if ($booking == 0) 
												{
													if ($gap >= $tottim) 
					...
					if ($hour == "") {
						...
						for ($intervl = 1; $intervl <= $serinterval; $intervl++) {
							...
							if ($hrs <= $day_time -> end_hour) {
								$bookinggg = $wpdb -> get_var($wpdb->prepare(
									'SELECT count(service_id) FROM ' . $wpdb -> prefix . sm_bookings . ' WHERE day = %d AND hour = %d AND minute = %d AND month = %d AND year = %d AND emp_id = %d AND status != "Disapproved" order by hour,minute ASC',
									intval($_REQUEST['cday']),
									$hrs,
									$mins,
									intval($_REQUEST['cmonth']),
					...
					$blktm = $hour . ":" . $minute . ":00";
					$blktime = $wpdb -> get_var($wpdb->prepare(
						'SELECT block_time FROM ' . $wpdb -> prefix . sm_block_time . ' WHERE  day = %d AND month = %d AND year = %d AND block_time = %s AND emp_id = %d',
						intval($_REQUEST['cday']),
						intval($_REQUEST['cmonth']),
						intval($_REQUEST['cyear']),
						$blktm,
					...
					{
						echo "<a class=\"hourschoice_res1 \" rel=\"1\"  href='#' >" . ($hour < 10 ? "0" . $hour : $hour) . ":" . ($minute < 15 ? "00" : $minute) . "</a>";
					} 
					else if ($bookinggg == 0) 
					{
						echo "<a class=\"hourschoice_res \" rel=\"1\" id=\"time_$time\" href='#' onclick='TimeClick(this);'>" . ($hour < 10 ? "0" . $hour : $hour) . ":" . ($minute < 15 ? "00" : $minute) . "</a>";
									...
									if (($service_gap_sh == $gap_hour) && ($service_gap_sm == $gap_minute)) {
										for ($j = 0; $j < $gap_interval; $j++) {
											...
											else 
											{
												...
												if ($blktime != "") 
												{
													echo "<a class=\"hourschoice_res1 \" rel=\"1\"  href='#' >" . ($hourcheck < 10 ? "0" . $hourcheck : $hourcheck) . ":" . ($minute < 15 ? $mntt : $minute) . "</a>";
												} 
												else if ($booking == 0) 
												{
													if ($gap >= $tottim) 
													...
													{
														echo "<a class=\"hourschoice_res1 \" rel=\"1\"  href='#'>" . ($hour < 10 ? "0" . $hourcheck : $hourcheck) . ":" . ($minute < 15 ? $mntt : $minute) . "</a>";
													}
												} 
												else
												{
