<?php
function backendbooking_bookingtab_callback(){
...
function nextButton01()
{
	...
	if(value1 != undefined || value1 != null)
	{
			...
			jQuery.ajax({
							...
							url: '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=backendbooking_bookingtab",
...
function nextButton04()
{
	...
	var lbdispser= unescape(serv);
		...
		{
		<?php
		$emailch = $wpdb->get_var($wpdb->prepare('SELECT email FROM ' . $wpdb->prefix . sm_clients . ' where id= %d', $_REQUEST['custId'])); ?>
									...
									<table class="activity_datatable" width="100%" border="0" cellspacing="0" cellpadding="8">
										<tbody>
											...
											<?php
														...
														$count =$wpdb->get_var(
															$wpdb->prepare(
																'SELECT count(' . $wpdb->prefix . sm_services .'.id) FROM ' . $wpdb->prefix . sm_services .' join '. $wpdb->prefix .sm_services_time .' on ' . $wpdb->prefix . sm_services . '.id=' .$wpdb->prefix .sm_services_time .'.service_id order by rank ASC'
															)
														);
