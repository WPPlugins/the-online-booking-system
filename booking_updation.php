<?php
function booking_updation_callback(){
global $wpdb;
$url = plugins_url('', __FILE__); 
 
 $admin_email = get_settings('admin_email');	
 $table_name = $wpdb->prefix . "sm_services";
 $booked_service_name = $wpdb->get_var($wpdb->prepare('SELECT name FROM ' . $table_name . ' WHERE id = %d', intval($_REQUEST['service'])));
 ...
 $emp_nam = $wpdb->get_var($wpdb->prepare('SELECT emp_name  FROM ' . $table_name . ' WHERE id = %d', intval($_REQUEST['emp'])));
 $emp_email = $wpdb->get_var($wpdb->prepare('SELECT email  FROM ' . $table_name . ' WHERE id = %d', intval($_REQUEST['emp'])));
	...
	if(isset($_REQUEST['email']))
	{
		if($_REQUEST['email']!="" &&  $_REQUEST['email']!="undefined")
		{
			...
			$client_name = $wpdb->get_var($wpdb->prepare('SELECT name  FROM ' . $table_name . ' WHERE email = %s ', esc_attr($_REQUEST['email'])));
		}
	}
	...
	if(isset($_REQUEST['mobileNum']))
	{
		...
		{
			$table_name = $wpdb->prefix . "sm_clients";
			$client_mobile = $wpdb->get_var($wpdb->prepare('SELECT mobile  FROM ' . $table_name . ' WHERE email = %s', esc_attr($_REQUEST['email'])));
		}
	}
	else
	{
    ...
    Booking Service Provider: $emp_nam";	
		...
		$empcolor=$wpdb->get_var($wpdb->prepare("SELECT emp_color FROM ". $wpdb->prefix . sm_employees. ' where id= %d', intval($_REQUEST['emp'])));
		$countt=$wpdb->get_var($wpdb->prepare("SELECT count(id) FROM ". $wpdb->prefix . sm_bookings. ' where StartTime = %s and EndTime  = %s and emp_id = %d and service_id  = %d and status = Approval Pending', $sttime, $etime, intval($_REQUEST['emp']), intval($_REQUEST['service'])));
        if($countt==0)
		{
