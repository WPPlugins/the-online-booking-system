<?php
function customer_booking_callback(){
...
else if($_REQUEST['method'] == 'updatecust')
{
...
else if($_REQUEST['method'] == 'deletecustomer')
{
			$bid = $wpdb->get_var($wpdb->prepare('SELECT count(id) FROM ' . $wpdb->prefix . sm_bookings . ' WHERE client_id = %d', intval($_REQUEST['delid'])));
			if($bid >0)
			{
				echo  "booked"; 
			}
...
else if($_REQUEST['method'] == 'searchcustomer')
{
			...
			if($vl == "")
			{
				$alpha  = "a";
				$query ="";
				$disp = $wpdb -> get_results($wpdb->prepare(
					"SELECT id, name, lastname, email, mobile, telephone, addressLine1, addressLine2, city,postalcode,country,comments FROM " . $wpdb -> prefix . sm_clients . " WHERE name LIKE %s order by name ASC",
					$alpha.'%'					
				));
			}
			else
			{
				$query ="";
				$disp = $wpdb -> get_results($wpdb->prepare(
					"SELECT id, name, lastname, email, mobile, telephone, addressLine1, addressLine2, city,postalcode,country,comments FROM " . $wpdb -> prefix . sm_clients . " WHERE name LIKE '%%%s%%' or lastname LIKE '%%%s%%' or email LIKE '%%%s%%' or mobile LIKE '%%%s%%' or telephone LIKE '%%%s%%' or city LIKE '%%%s%%' or postalcode LIKE '%%%s%%' or country LIKE '%%%s%%'  order by name ASC",
					$vl,
					$vl,
					$vl,
					$vl,
			...
			<td>
				<span class="data_actions iconsweet">
					<a class="tip_north colorbox" original-title="Edit Customer" id="li_<?php echo $i;?>" href="#Editcustomer" style="cursor:pointer;"  onclick="getappointments(this);">C</a>
					<a class="tip_north colorbox" original-title="Bookings" id="e_<?php echo $i;?>" onClick="custdetail(this)" href="#customerdetail" style="cursor:pointer;">P</a>
					<a class="tip_north colorbox" original-title="Send Email" id="h_<?php echo $i;?>" onClick="emailcust(this)" href="#customemail" style="cursor:pointer;">A</a>
...
else if($_REQUEST['method'] == 'sortingchar')
 {
			...
			$disp = $wpdb -> get_results($wpdb->prepare(
				"SELECT id, name, lastname, email, mobile, telephone, addressLine1, addressLine2, city,postalcode,country,comments FROM " . $wpdb -> prefix . sm_clients . " WHERE name LIKE %s or lastname LIKE %s order by name ASC",
				$ch.'%',
				$ch.'%'
			));
			$count = count($disp);
...
else if($_REQUEST['method'] == 'customerHistory')
{
...
<table class="activity_datatable"  width="100%" border="0" cellspacing="0" cellpadding="8">
									<tbody>
										...
										<?php 
											...
											$comments = $wpdb->get_var($wpdb->prepare('SELECT note FROM ' . $wpdb->prefix . sm_bookings . ' WHERE id = %d', $bookings[$i]->id));
											$empname = $wpdb->get_var($wpdb->prepare('SELECT emp_name FROM ' . $wpdb->prefix . sm_employees . ' WHERE id = %d', $bookings[$i]->emp_id));
											$sercurrency = $wpdb -> get_var($wpdb->prepare('SELECT currency_sign FROM ' . $wpdb -> prefix . sm_currency . ' where ' . $wpdb -> prefix . sm_currency . '.currency_used = 1'));
											?>
										<tr>
											...
											<td><span class="data_actions iconsweet"><a class="tip_north colorbox" original-title="Note" id="<?php echo $bookings[$i]->id; ?>"  href="#notecustomer" onclick="comen(this)"  style="cursor:pointer;">C</a>
...
die;	
...
else if($_REQUEST['method'] == 'commentcustomer')
	{
			$wpdb->query
		    (
		            $wpdb->prepare
...
else if($_REQUEST['method'] == 'notecustomer')
	{
			$comments = $wpdb->get_var($wpdb->prepare('SELECT note FROM ' . $wpdb->prefix . sm_bookings . ' WHERE id = %d', intval($_REQUEST['custoId'])));
			?>
...
else if($_REQUEST['method'] == 'emailcustomers')
	{
				$email = $wpdb->get_var($wpdb->prepare('SELECT email FROM ' . $wpdb->prefix . sm_clients . ' WHERE id = %d', intval($_REQUEST['id'])));
				$to = $email;
	...
	else if($_REQUEST['method'] == "checkemaill")
	{
		$email = esc_attr($_REQUEST['email']);
		$ckemail  =  $wpdb->get_var($wpdb->prepare('SELECT count(id)  FROM ' . $wpdb->prefix . sm_clients . ' where email = %s', $email));
		if($ckemail==0)
		{
			echo "newcustomer";
		}
...
else if($_REQUEST['method'] == "delbooking")
	{
...
function deletebooking(b)
{
 ...
 var id=b.id;
	...
	jQuery.ajax
			({
				type: "POST",
				data:"id="+id+"&method=delbooking",
				url: '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=customer",
...
function emailcustomer()
{
		...
		{											
			...
			({
				type: "POST",
				data:"id="+ eid +"&ecut="+ecut+"&method=emailcustomers",
				url: '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=customer_booking",
				success: function(data)
				{
...
function comment()
{
		...
		{											
			...
			({
				type: "POST",
				data:"id="+ id +"&comment="+comm+"&method=commentcustomer",
				url: '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=customer_booking",
				success: function(data)
				{
						...
						jQuery.colorbox.close();
														...
														function comen(e)
														{
																...
																jQuery(document).ready(function($) 
																{
																	...
																	({
																		type: "POST",
																		data:"custoId="+id+"&method=notecustomer",
																		url: '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=customer_booking",
																		success: function(data)
																		{
												...
												function change()
												{
															...
															jQuery("#customeremail").removeClass("in_error");
																...
																function savecustomer()
																{
																	if(customerFirstNameblur())
																	{
																				...
																				if(emialreq==false)
																				{
																					...
																					var address = jQuery("#customeremail").val();
																					 ...
																					 jQuery.ajax
																						({
																								type: "POST",
																								data: "email="+address+"&method=checkemaill",
																								url: '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=customer",
																								success: function(data) 
																								{
																				...
																				<?php
																				$sell = $wpdb -> get_var($wpdb->prepare('SELECT name FROM ' . $wpdb -> prefix . sm_cuntry . ' where used ="1"'));
																				?>
																				...
																				({
																						type: "POST",
																						data:"firstnam="+firstnam+"&lastname="+lastnam+"&email="+email+"&mobile="+mobile+"&tel="+tel+"&customerAddress1="+customerAddress1+"&customerAddress2="+customerAddress2+"&customerCity="+customerCity+"&Country="+Country+"&customerZip="+customerZip+"&comments="+comments+"&method=addcustomer",
																						url: '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=customer",
																						success: function(data) 
																						{
																								...
																								({
																									...
																									url: '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=customersrebind",
																									success: function(data)
																									{
																											var temp=data;
													...
													function searchcustomers(e)
													{
														jQuery(document).ready(function($) 
														{
															...
															({
																type: "POST",
																data:"searchstring="+searchstring+"&method=searchcustomer",
																url: '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=customer_booking",
																success: function(data)
																{
																	...
																	jQuery('.colorbox').colorbox({inline: true});
																}
															});
														});
													}
													...
													function updatecustomer()
													{
														if(customerFirstNameblur1())
														{
															jQuery(document).ready(function($) 
															{
																...
																({	
																		type: "POST",
																		data:"firstnam1="+firstnam1+"&lastnam1="+lastname1+"&email1="+email1+"&id1="+iddd1+"&mobile1="+mobile1+"&tel1="+tel1+"&customerAddress11="+customerAddress11+"&customerAddress21="+customerAddress21+"&customerCity1="+customerCity1+"&Country1="+Country1+"&customerZip1="+customerZip1+"&comments1="+comments1+"&method=updatecust",
																		url: '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=customer_booking",
																		success: function(data)
																		{
														...
														jQuery.ajax({
																type: "POST",
																data:"delid="+delid+"&method=deletecustomer",
																url: '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=customer_booking",
																success: function(data)
																{
														...
														function custdetail(e)
														{
															...
															jQuery(document).ready(function($)
															{
																...
																({
																		type: "POST",
																		data:"custId="+cid+"&method=customerHistory",
																		url: '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=customer_booking",
																		success: function(data)
																		{
														...
														function bookbackend(h)
														{
															...
															jQuery('#booking_backend_tab').load('<?php echo admin_url('admin-ajax.php'); ?>' + "?action=backendbooking_bookingtab"+"&custId="+hid,function(){jQuery('#loading').remove();jQuery('.maincontainer1').css('display', 'block');});
													...
													jQuery('.backdrop').click(function(){
														...
														function sorting(e)
														{
															...
															jQuery(document).ready(function($) 
															{
																...
																({
																			type: "POST",
																			data:"charcter="+delid+"&method=sortingchar",
																			url: '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=customer_booking",
																			success: function(data)
																			{
...
<script type="text/javascript">jQuery("select, input:checkbox, input:radio").uniform();</script>											
	...
	<div  class="contentarea">
		...
		<ul class="form_fields_container" style="margin-bottom:0px;margin-top:10px">
				<li>
					<a class="greyishBtn button_small colorbox"  href="#Addcustomer">Add New Customer</a>
...
?></tr>
	...
	<div class="widget-wp-obs">
						...
						<table class="activity_datatable" id="searchcustomer" width="100%" border="0" cellspacing="0" cellpadding="8">
									<tbody>
										...
										<ul class="customer_list_content" id="customerListContainer" onscroll="getMoreCustomersByScroll();" >
											...
											$disp = $wpdb -> get_results($wpdb->prepare("SELECT id, name, lastname, email, mobile, telephone, addressLine1, addressLine2, city,postalcode,country,comments FROM " . $wpdb -> prefix . sm_clients  . " WHERE name LIKE 'a%' order by name ASC"));
											$cou = count($disp);
											?>
			...
			<div id="Addcustomer"  style="width:700px;">
										...
										<li style="padding-bottom:9px">
											...
											<div class="form_input"  style="width:60%">
												<select name="Country" id="Country">
													...
													$sel_country = $wpdb -> get_var($wpdb->prepare('SELECT name  FROM ' . $wpdb -> prefix . sm_cuntry . ' where used = 1'));
				...
				<div id="Editcustomer"  style="width:700px;">
						...
						<div class="one_two_wrap fl_right">
											...
											<div class="form_input"  style="width:60%">
												<select name="Country1" id="Country1" style="">
													<?php
														...
														$country = $wpdb->get_var($wpdb->prepare('SELECT country FROM ' . $wpdb->prefix . sm_clients . ' where id = %d', $_REQUEST['id']));      
														if ($country !=0) 
														{
													?>
							...
							<div id="customerdetail"  style="width:850px;">
								<div class="widget-wp-obs" style="margin:5px;">
									<div class="widget-wp-obs_title">
										<span class="iconsweet">o</span>
			...
			<div id="notecustomer"  style="width:600px;">
			<div class="widget-wp-obs" style="margin:5px;">
		...
		<div id="customemail"  style="width:600px;">
