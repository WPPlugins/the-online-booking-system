...
if($_REQUEST['method'] == 'AddService')
{
		        ...
		        (
		                  $wpdb->prepare
		                  (
		                       "INSERT INTO ".$wpdb->prefix."sm_services(name,cost,rank) VALUES( %s, %f, %d)",
		                       $sname,
		                      $_REQUEST['cost'],
		                       $sid
		                   )
...
else if($_REQUEST['method'] == 'deleteservice')
{
				$id = $wpdb -> get_var($wpdb->prepare('SELECT count(id) FROM ' . $wpdb -> prefix . sm_allocate_serv . '  where serv_id = %d', intval($_REQUEST['data'])));
				if($id!=0)
				{
						echo "exists";
				}
...
else if($_REQUEST['method'] == 'editrank')
{
			$id = $wpdb->get_results
			(
					$wpdb->prepare
...
else if($_REQUEST['method'] == 'updateservice')
{
			...
			$wpdb->query
	     	(
	            $wpdb->prepare
	            (
	                    "UPDATE ".$wpdb->prefix."sm_services SET name = %s , cost = %f WHERE id = %d",
	                    $sername,
	                  $_REQUEST['cost'],
	                    intval($_REQUEST['id'])
	             )
...
else if($_REQUEST['method'] == 'serviceid')
{
			global $wpdb;
			$table_name = $wpdb->prefix . "sm_services";
			$sid = $wpdb->get_var($wpdb->prepare('SELECT count(id) FROM ' . $wpdb->prefix . sm_services));
			echo $sid;
}
else
{
	...
	function add_wizservice()
				{	
		             ...
		             {  
											...
											({
		                            			type: "POST",
		                                        data: "method=serviceid",
		                                     	url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=services",
						  ...
						  if(addServiceNameBlur() && addServiceCostBlur())
						  {
				                  ...
				                  if(flag)
		                 		  {                         
		                        		...
		                        		({
		                            			type: "POST",
		                                        data: "hour="+hour+"&mint="+minut+"&name="+name+"&cost="+cst+"&end_g_hour="+ehour+"&end_g_minute="+emin+"&start_g_hour="+shour+"&start_g_minute="+smin+"&method=AddService",
		                                     	url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=services",
		                                        success: function(data) 
		                                        {    
													if(ser<=1)
													...
													jQuery("#errormsg").css('display','none');  
		                                                ...
		                                                setTimeout(function() 
		                                                {
																...
																({
		                                                    			type: "POST",
		                                                                data: "",
		                                                                url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=servicerebind",
		                                                                success: function(data) 
																		{  	
																				jQuery("#errormsg").css('display','none');
																				var temp=data;
													...
													else
													{
														...
														jQuery('#LimitedContentPlaceHolder').fadeOut("fast").load('<?php echo admin_url('admin-ajax.php'); ?>' + "?action=services").fadeIn(1000);
		...
		function delete_service(service) 
		{
				...
				action = confirm("Are you sure you want to delete this service?");
					if(action == true)
					{
						...
						({
								type: "POST",
								data: "data=" + service + "&method=deleteservice",
								url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=services",
								success: function(data) 
								{
								if(data=='exists')
								{
									...
									({
											...
											success: function(data)
											{
												window.location.href = url1+"/wp-admin/admin.php?page=TabServices";
											}
									});
								}
						});
			...
			function editser()
			{
					...
					({
						type: "POST",
						data: "id="+id+"&name="+namee+"&cost="+cost+"&method=updateservice",
						url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=services",
						success: function(data) 
						{
				...
				<?php
					$serc = $wpdb -> get_var($wpdb->prepare('SELECT count(id) FROM ' . $wpdb -> prefix . sm_services));
				 ?>
				...
				function submitrank()
				{
					for(i = 0; i <= servicecount; i++)
					{
						...
						({
							type: "POST",
							data: "rnk_id="+value+"&serviceId="+serviceId+"&method=editrank",
							url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=services",
							success: function(data)
							{
...
<div id="servicediv">
		...
		<div class="one_wrap fl_left">
				...
				<ul class="form_fields_container">
					...
					<?php 
				global $wpdb;
				$seridd = $wpdb -> get_var($wpdb->prepare('SELECT count(id) FROM ' . $wpdb -> prefix . sm_services));
				?>
					<li>
						<?php
						if($seridd<3)
						{
						?>
							<a  id="submit" onclick="return submitrank();" href="javascript:;" class="greyishBtn button_small" >Save Services</a>
							<a id="modalnewservices"  href="#modalNewService" class="greyishBtn button_small colorbox" >Add New Services</a>
			...
			<ul class="form_fields_container">
				...
				<li>
						<a href="http://www.wp-online-booking-system.com" target="_blank"><img style="margin-left: -12px;" src="<?php echo $url;?>images/unlimited_employees_service.jpg" ></a>
		...
		<div style="width:600px;" id="modalNewService">
										...
										<div class="widget-wp-obs" style="margin:5px;">
											...
											<div class="widget-wp-obs_body">
												<ul class="form_fields_container">
													...
													$curr = $wpdb -> get_var($wpdb->prepare('SELECT currency_sign FROM ' . $wpdb -> prefix . sm_currency . '  where currency_used = "1" '));
													?>
													<li style="padding-bottom: 5px">
...
<div  style="width:600px" id="modaleditService">
