...
<script>
	...
	if(minformat==1)
	{
		filepath= '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=calendar";
	}
	else
	{
		filepath= '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=calendar30";
	}
	var dat = new Date();
	...
	jQuery("#prev_month").live('click',function() 
	{
...
function DayClick(e)
	{
			...
			({
				type : "POST",
				data : "cmonth=" + month + "&cyear=" + year + "&cday=" + day + "&cservice=" + service_id + "&empId=" + emp + "&method=EmpTimmings",
				url : filepath,
				success : function(data) 
				{
