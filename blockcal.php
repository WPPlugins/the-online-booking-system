...
for($list_day = 1; $list_day <= $days_in_month; $list_day++):
			...
			$dattimeee = $wpdb-> get_var(
				$wpdb->prepare(
					'SELECT count(day) FROM ' . $wpdb -> prefix . sm_block_date . ' where month=%d and day = %d and year= %d and emp_id= %d order by day ASC',
					$month,
					$list_day,
					$year,
					$val
			...
			$count = $wpdb-> get_var(
				$wpdb->prepare(
					'SELECT count(id) FROM ' . $wpdb -> prefix . sm_timings . ' WHERE day = %d AND  emp_id = %d AND  blocked = %d',
					$selected_day,
					$val,
					1	
				)
