		...
		jQuery("#strthrrr").val(hrselect);
						 ...
						 function populateemployees()
						 {
							...
							jQuery.ajax({
								...
								url: '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=getlasthour",
									...
									if(flag!=1)
									{
										...
										else
										{
											...
											({
													...
													url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=book",
													success: function(data) 
													{
	...
	<div class="widget-wp-obs_title">
		...
		<ul class="form_fields_container">
			<li>
				  ...
				  $jid=$wpdb->get_var($wpdb->prepare("select service_id from ".$table_name." where id = %d", intval($_REQUEST['id'])));
			...
			<li>
		 		  <label>Client Name :</label>
					 ...
					 <?php
					   ...
					   $table_name = $wpdb->prefix . "sm_bookings";
					   $clid=$wpdb->get_var($wpdb->prepare("select client_id from ".$table_name." where id = %d", intval($_REQUEST['id'])));
					   $clientnam = $wpdb->get_var($wpdb->prepare("SELECT name FROM ". $wpdb->prefix . sm_clients . " where id = %d", $clid));
					   $clientlnam = $wpdb->get_var($wpdb->prepare("SELECT lastname FROM ". $wpdb->prefix . sm_clients . " where id = %d", $clid));
						?>
					...
					<?php
			           $table_name = $wpdb->prefix . "sm_bookings";
					   $clid=$wpdb->get_var($wpdb->prepare("select client_id from ".$table_name." where id = %d", intval($_REQUEST['id'])));
						$clientemail = $wpdb->get_var($wpdb->prepare("SELECT email FROM ". $wpdb->prefix . sm_clients . " where id = %d", $clid));
					   ...
					   $table_name = $wpdb->prefix . "sm_bookings";
					   $clid=$wpdb->get_var($wpdb->prepare("select client_id from ".$table_name." where id = %d", intval($_REQUEST['id'])));
						$clientmobile = $wpdb->get_var($wpdb->prepare("SELECT mobile FROM ". $wpdb->prefix . sm_clients . " where id = %d", $clid));
				    ...
				    $table_name = $wpdb->prefix . "sm_bookings";
					$StartTime=$wpdb->get_var($wpdb->prepare("select StartTime from ".$table_name." where id = %d", intval($_REQUEST['id'])));
			...
			<label>Status :</label>
			 <div class="form_input">
			<input type="hidden" id="bookid" value="<?php echo sanitize_text_field($_REQUEST['id']);?>" />
			<select id="bookingstatus" name="bookingstatus">
					<?php
		          
				  $table_name = $wpdb->prefix . "sm_bookings";
				  $sta=$wpdb->get_var($wpdb->prepare("select status from ".$table_name." where id = %d", intval($_REQUEST['id'])));
	 ...
	 <input type="hidden" id="bookst" value="<?php echo $sta; ?>" />
		...
		if($tt!=0)
		{
				  $eimd = $wpdb->get_var($wpdb->prepare("SELECT emp_id FROM ". $wpdb->prefix . sm_bookings . " where id= %d", $tt));
				?>
	...
	?>
		<div class="widget-wp-obs" style="margin:5px;">
			...
			<li>
				  ...
				  <label>Service Name :</label>
				  <div class="form_input">
				  		<input type="text" value="<?php echo sanitize_text_field($_GET['sernm']); ?>" disabled="disabled"></input>
				  </div>
			</li>
			<li>
					<?php
					$empnm=$wpdb->get_var($wpdb->prepare("SELECT emp_name FROM ". $wpdb->prefix . sm_employees. ' where id= %d', intval($_GET['empid'])));
					?>    
		          <input type="hidden" id="enddate" name="enddate"/>	
				  <label>Employee :</label>
				  <div class="form_input">
