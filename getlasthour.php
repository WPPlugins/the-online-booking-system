...
if(isset($_REQUEST['popultemp']))
{
			...
			$emppid=$wpdb->get_var($wpdb->prepare("select emp_id from " . $wpdb->prefix . "sm_bookings" . " where id = %d", intval($_REQUEST['empidd'])));
...
else if(isset($_REQUEST['day']))
{
		...
		$dattimeee = $wpdb->get_var($wpdb->prepare('SELECT count(day) FROM ' . $wpdb->prefix . sm_block_date . ' WHERE day = %d AND month = %d AND year = %d AND emp_id = %d',
			intval($_REQUEST['dayy']),
			intval($_REQUEST['month']),
			intval($_REQUEST['year']),
			intval($_REQUEST['empid'])
									...
									if (($service_gap_sh == $gap_hour) && ($service_gap_sm == $gap_minute)) {
										for ($j = 0; $j < $gap_interval; $j++) {
											...
											else 
											{
												...
												$booking = $wpdb -> get_var($wpdb->prepare('SELECT count(service_id) FROM ' . $wpdb -> prefix . sm_bookings . ' WHERE day = %d AND hour = %d AND minute = %d AND month = %d AND year = %d AND emp_id = %d AND status != "Disapproved"  order by hour,minute ASC',
													intval($_REQUEST['dayy']),
													$hour,
													$minute,
													intval($_REQUEST['month']),
												...
												$blktime = $wpdb -> get_var($wpdb->prepare('SELECT block_time FROM ' . $wpdb -> prefix . sm_block_time . ' WHERE  day = %d AND month = %d AND year = %d AND block_time = %s AND emp_id =  %d',
													intval($_REQUEST['dayy']),
													intval($_REQUEST['month']),
													intval($_REQUEST['year']),
													$blktm,
					...
					else 
					{
						for ($intervl = 1; $intervl <= $serinterval; $intervl++) 
						{
							...
							if ($hrs <= $day_time -> end_hour) 
							{
								...
								$bookinggg = $wpdb -> get_var($wpdb->prepare('SELECT count(service_id) FROM ' . $wpdb -> prefix . sm_bookings . ' WHERE day = %d AND hour = %d AND minute = %d AND month = %d AND year = %d AND emp_id = %d AND status != "Disapproved" order by hour,minute ASC',
									intval($_REQUEST['dayy']), 
									$hrs,
									$mins,
									intval($_REQUEST['month']),
