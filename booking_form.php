...
function save_fields()
{
			...
			for(var i=0;i<countfields;i++)
			{
					...
					({
							...
							url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=radio",
			...
			for(var k=0;k<countfields;k++)
			{
					...
					({
							type: "POST",
							data: "translate="+translate+"&value="+valu+"&method=transdata",
							url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=booking_form",
							success: function(data)
							{
							}
					});
...
<div class="one_wrap fl_left" >
			...
			<ul class="form_fields_container" style="margin-left:20px;margin-top:0px;">
				<li>
					<a id="button" href="javascript:;" style="margin-left:20px;" onclick="return save_fields();" class="greyishBtn button_small">Save Fields</a>
				</li>
			</ul>
		</div>
<script type="text/javascript">
