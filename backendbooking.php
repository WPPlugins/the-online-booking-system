...
function backendbooking_callback(){
...
jQuery("select, input:checkbox, input:radio").uniform(); 
		...
		<tbody>
			<tr >
				...
				$count= $wpdb->get_var(
					$wpdb->prepare(
						'SELECT count('. $wpdb->prefix . sm_employees . '.id) FROM ' . $wpdb->prefix . sm_employees .'  join '. $wpdb->prefix .sm_allocate_serv .' on ' . $wpdb->prefix . sm_employees . '.id=' .$wpdb->prefix .sm_allocate_serv .'.emp_id where  ' .$wpdb->prefix .sm_allocate_serv . '.serv_id=%d and ' . $wpdb->prefix . sm_employees . '.status = "Active"',
						$sidd
					)
				);
				for($i=0;$i<$count;$i++)
...
{
	$msg1  =  $wpdb->get_var($wpdb->prepare('SELECT translate  FROM ' . $wpdb->prefix . sm_translation . ' where id = %d', 17));
	$msg2  =  $wpdb->get_var($wpdb->prepare('SELECT translate  FROM ' . $wpdb->prefix . sm_translation . ' where id = %d', 18));
	...
	if(value1 != undefined || value1 != null)
	 {
			...
			jQuery.ajax({
					type: "POST",
					data: "serviceId="+value1+"&method=GetEmployees",
					url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=backendbooking",
...
function nextButton02()
{
	...
	if(value03 != undefined || value03 != null)
	{
		...
		<?php 
			$minformat  =  $wpdb->get_var($wpdb->prepare('SELECT minuteformat  FROM ' . $wpdb->prefix . sm_settings . ' where id = %d', 1));
		?>
...
function nextButton03()
{
		...
		function bookItNow1()
		{
			...
			$emailch = $wpdb->get_var($wpdb->prepare('SELECT email FROM ' . $wpdb->prefix . sm_clients . ' where id=%d', intval($_REQUEST['custId']))); ?>
			...
			jQuery(document).ready(function($) 
			{
				...
				jQuery.ajax({
							...
							url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=sendmail",
							success: function(data) 
							{
		...
		<div class="superContainer1">
			<div class="maincontainer1" style="display:none;">
				...
				<?php $table_name = $wpdb -> prefix . "sm_settings";
					...
					<?php
								$thanks = $wpdb->get_var($wpdb->prepare('SELECT thanks from ' . $wpdb->prefix . sm_settings . ' where id=%d', 1));
								$trans = $wpdb->get_results
								(
										$wpdb->prepare
										(
									...
									<table class="activity_datatable" width="100%" border="0" cellspacing="0" cellpadding="8">
										<tbody>
											...
											<?php
														...
														$cur_icon =  $wpdb->get_var($wpdb->prepare('SELECT currency_sign  FROM ' . $wpdb->prefix . sm_currency . ' where currency_used = %d', 1));
														for( $i = 0; $i < $count; $i++)
														{
														$idd=$hour[$i]->id;
														$nam=$hour[$i]->name;
					...
					<div class="one_wrap fl_left" id="step401" style="display:none">
						...
						<div class="widget-wp-obs" style="margin-top:5px;margin-bottom:0px">
							...
							<div class="widget-wp-obs_body">
								<ul class="form_fields_container">
									<?php
										$fname = $wpdb->get_var($wpdb->prepare('SELECT name FROM ' . $wpdb->prefix . sm_clients . ' where id= %d', intval($_REQUEST['custId'])));
										$lname = $wpdb->get_var($wpdb->prepare('SELECT lastname FROM ' . $wpdb->prefix . sm_clients . ' where id= %d', intval($_REQUEST['custId'])));
										$eml = $wpdb->get_var($wpdb->prepare('SELECT email FROM ' . $wpdb->prefix . sm_clients . ' where id= %d', intval($_REQUEST['custId'])));
										$tel = $wpdb->get_var($wpdb->prepare('SELECT telephone FROM ' . $wpdb->prefix . sm_clients . ' where id= %d', intval($_REQUEST['custId'])));
										$add1 = $wpdb->get_var($wpdb->prepare('SELECT addressLine1 FROM ' . $wpdb->prefix . sm_clients . ' where id= %d', intval($_REQUEST['custId'])));
										$add2 = $wpdb->get_var($wpdb->prepare('SELECT addressLine2 FROM ' . $wpdb->prefix . sm_clients . ' where id= %d', intval($_REQUEST['custId'])));
										$cty = $wpdb->get_var($wpdb->prepare('SELECT city FROM ' . $wpdb->prefix . sm_clients . ' where id= %d', intval($_REQUEST['custId'])));
										$post = $wpdb->get_var($wpdb->prepare('SELECT postalcode FROM ' . $wpdb->prefix . sm_clients . ' where id= %d', intval($_REQUEST['custId'])));
										$cntry = $wpdb->get_var($wpdb->prepare('SELECT country  FROM ' . $wpdb->prefix . sm_clients . ' where id= %d', intval($_REQUEST['custId'])));
										$mobile = $wpdb->get_var($wpdb->prepare('SELECT mobile FROM ' . $wpdb->prefix . sm_clients . ' where id= %d', intval($_REQUEST['custId'])));
