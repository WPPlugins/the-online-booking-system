...
{
	if($_REQUEST['method'] == 'AddEmployees')
	{
	...
	else if ($_REQUEST['method'] == 'UpdateEmployees')
	{
	...
	else if ($_REQUEST['method'] == 'deleteemployee') 
	{
			
			$bokid= $wpdb->get_var($wpdb->prepare('SELECT count(id) FROM ' . $wpdb->prefix . sm_bookings . ' where emp_id = %d', intval($_REQUEST['data'])));
			$allocid= $wpdb->get_var($wpdb->prepare('SELECT count(id) FROM ' . $wpdb->prefix . sm_allocate_serv . ' where emp_id = %d', intval($_REQUEST['data'])));
			if($bokid!=0)
			{
				echo "booking";
			}
	...
	else if($_REQUEST['method'] == 'AllocateEmployees')
	{
			...
			$allservices = explode(",", $allserv);
				...
				{
					$name = $allservices[$c];
					$servid = $wpdb -> get_var($wpdb->prepare('SELECT id FROM ' . $wpdb -> prefix . sm_services . '  where name = %s', $name));
					$eeid = $wpdb -> get_var($wpdb->prepare('SELECT emp_id FROM ' . $wpdb -> prefix . sm_allocate_serv . '  where emp_id = %d and  serv_id = %d', $empid,$servid));
	...
	else if($_REQUEST['method'] == 'WorkingHours')
	 {
...
else if($_REQUEST['method'] == 'delBlockTime')
	{
	...
	else if($_REQUEST['method'] == 'AddBlockTime')
	{
		$timefrt1 = $wpdb -> get_var($wpdb->prepare('SELECT TimeFormat   FROM ' . $wpdb -> prefix . sm_settings));
		if($timefrt1=="1")
		{
	...
	else if($_REQUEST['method'] == "BlockTimmings")
	{
		$curr =  date("Y/m/d");
	...
	else if($_REQUEST['method'] =="SaveWorkingHours")
	{
...
function savehour() 
			...
			if(empval != 0)
			{
				for(var i=0;i<7;i++)
				{
						...
						switch(i)
						{
							...
							jQuery.ajax({
									type: "POST",
									data: "day="+dayy+"&start_hours="+strtmonhours+"&start_minutes="+strtmonmin+"&end_hours="+endmonhours+"&end_mins="+endmonmin+"&empid="+empval+"&flag="+flag+"&method=SaveWorkingHours",
									url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=employees",
	...
	function add_employee()
	{
				...
				{		var nam = document.getElementById("employeeName1").value;
						...
						jQuery.ajax({
									type: "POST",
									data: "ename="+name1+"&ecode="+code1+"&input_field_1="+input_field_1+"&ephone="+phone1+"&email="+email1+"&rad="+value1+"&method=AddEmployees",
									url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=employees",
									success: function(data) 
									{
	...
	function delete_employee(service) 
	{
		...
		if(action == true)
		{
				jQuery.ajax({
				type: "POST",
				data: "data=" + service+"&method=deleteemployee",
				url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=employees",
				success: function(data) 
				{
				...
				var urlSite = "<?php echo site_url(); ?>";
							jQuery.ajax({
										...
										url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=employeesrebind",
										success: function(data)
										{
											window.location.href = urlSite +"/wp-admin/admin.php?page=TabEmployees";
										}
	...
	function update()
	{
		...
		if(editEmployeeNameBlur() && editEmployeeEmailBlur() && editEmployeePhoneBlur())
		{
			...
			jQuery.ajax({
						type: "POST",
						data: "id="+id+"&em="+em+"&empn="+empnam+"&ph="+ph+"&col="+col+"&value="+value+"&method=UpdateEmployees",
						url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=employees",
						success: function(data) 
						{
							jQuery.ajax({
	...
	function callemp()
	{
				...
				({
						type: "POST",
						data: "emp_id="+empid+"&method=BindEmployees",
						url: '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=employee_allocation",						
						success: function(data) 
						{
	...
	function get_emp_timingswiz()
	{
			...
			jQuery.ajax({
			type: "POST",
			data: "emp_id="+empid+"&method=WorkingHours",
			url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=employees",
			success: function(data) 
			{
				jQuery("#disptb").empty();
...
function timeoff()
{
	...
	if(document.getElementById('radioblockhours').checked==false)
	{
					...
					({
							...
							url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=block_cal",
							success: function(data) 
							{
								...
								if(valemp!=0)
								{
									...
									({
									        type: "POST",
									        data: "cmonth="+month+"&cyear="+year+"&cday="+day+"&empnam="+sda,
									        url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=blocktim",
	        								success: function(data) 
											{
...
function save_block_time()
{
		...
		({
			type: "POST",
			data: "day="+day+"&month="+curmonth+"&empid="+empid+"&year="+yer+"&method=delBlockTime",
			url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=employees",
			success: function(data)
			{
				...
				for(var j=0; j<len; j++)
				{
					var tmp1 = sda.options[j].value;
			    		...
			    		({
							type: "POST",
							data: "day="+day+"&month="+curmonth+"&empid="+empid+"&year="+yer+"&strttm="+tmp1+"&method=AddBlockTime",
							url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=employees",
							success: function(data)
							{
...
function block_dat() 
{
	jQuery(document).ready(function($) 
	{
		...
		({
			type: "POST",
			data: "to="+blokdate+"&from="+blokdat+"&emppid="+emmpid+"&method=BlockTimmings",
			url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=employees",
			success: function(data) 
			{
...
function savedetails(opt)	
{
		...
		var empid=document.getElementById('all_emppp').value;
		    ...
		    jQuery.ajax({
					type: "POST",
					data: "emp="+empid+"&allserv="+strSel+ "&method=AllocateEmployees",
					url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=employees",
					success: function(data) 
					{
			...
			<div class="msgbar msg_Success hide_onC" id="savedit" style="display: none;margin-top:5px;margin-bottom:5px;width:95%;margin-left:5px;">
				...
				<ul class="form_fields_container" style="margin-top:10px;">
					...
					<?php
						...
						{
						?>
						<a class="greyishBtn button_small colorbox"  href="#Addemployee" >Add New Employee</a>
						<?php
						}
						?>
						<a class="greyishBtn button_small colorbox"  href="#allocateEmployeeee">Allocate Services</a>
						<a class="greyishBtn button_small colorbox"  href="#officeHours">Working Hours</a>
						<a class="greyishBtn button_small colorbox"  href="#timeOff">Time Off</a>
					</li>
				</ul>
							...
							<tbody>
								...
								<tr class="tyu">
									...
									<label id="empsts_<?php echo $i; ?>" style="color:green;cursor:default;"><?php echo $status;
										?></label><?php }
									?></td>
									<td><span class="data_actions iconsweet"> <a class="tip_north colorbox" original-title="Edit" id="<?php echo $i;?>" href="#Editemployee" onclick="get_edit_emp(this)"   style="cursor:pointer;">C</a> <a class="tip_north" original-title="Delete" id='<?php echo $idd;?>'  href="javascript: delete_employee('<?php echo $idd;?>')"  style="cursor:pointer;">X</a> </span></td>
				...
				<ul>
				<li>
					<a href="http://www.wp-online-booking-system.com" target="_blank"><img style="margin-left: -12px;margin-top: 10px;" src="<?php echo $url;?>images/unlimited_employees_service.jpg" ></a>
				</li>
				</ul>
				</div>
				<div style="display:none;">
				<div id="Addemployee"  style="width:600px;">
					...
					<div  style="width:600px;" id="allocateEmployeeee">
					...
					<div  id="officeHours" style="width:850px;">
					...
					<div  id="timeOff" style="width:550px;">
					...
					<div style="width:600px;" id="Editemployee">
...
<?php include("block_cal.php");blockcal_callback(); ?>
<?php }}?>