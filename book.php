...
$url = plugins_url('', __FILE__); 
  ...
  else
  {
		...
		$empcol = $wpdb->get_var($wpdb->prepare('SELECT emp_color FROM ' . $table_name . ' WHERE id = %d', $empid));
		$mystring=$strtdat;
		$mystring.=" ";
		$mystring.=$hour;
		$mystring.=":";
		...
		$booked_service_hrs = $wpdb->get_var($wpdb->prepare('SELECT hours FROM ' . $table_name . ' WHERE service_id = %d', $serid));
		$booked_service_min = $wpdb->get_var($wpdb->prepare('SELECT minutes FROM ' . $table_name . ' WHERE service_id = %d', $serid));
	...
	if($status=="Approved" && $emi==1)
	{
	     ...
	     (
      	 ...
      	 $client_id1 = $wpdb->get_var($wpdb->prepare("SELECT client_id From  " . $wpdb->prefix . sm_bookings . " WHERE id = %d", $edtid));
		 $service_id = $wpdb->get_var($wpdb->prepare("SELECT service_id From  " . $wpdb->prefix . sm_bookings . " WHERE id = %d", $edtid)); 
         $empid=$wpdb->get_var($wpdb->prepare("SELECT emp_id From  " . $wpdb->prefix . sm_bookings . " WHERE id = %d", $edtid));
		 ...
		 $day = $wpdb->get_var($wpdb->prepare("SELECT day From  " . $wpdb->prefix . sm_bookings . " WHERE id = %d", $edtid));			
		 $month = $wpdb->get_var($wpdb->prepare("SELECT month From  " . $wpdb->prefix . sm_bookings . " WHERE id = %d", $edtid));	  
		 $year = $wpdb->get_var($wpdb->prepare("SELECT year From  " . $wpdb->prefix . sm_bookings . " WHERE id = %d", $edtid));
		 $hour = $wpdb->get_var($wpdb->prepare("SELECT hour From  " . $wpdb->prefix . sm_bookings . " WHERE id = %d", $edtid)); 
	     $minute = $wpdb->get_var($wpdb->prepare("SELECT minute From  " . $wpdb->prefix . sm_bookings . " WHERE id = %d", $edtid));
		 ...
		 $ser_name = $wpdb->get_var($wpdb->prepare("SELECT name From  " . $wpdb->prefix . sm_services . "  WHERE id = %d", $serid));		 
	...
	if($status=="Disapproved" && $emi==1)
	{
		...
		echo "Disapproved";	   
        $admin_email = get_settings('admin_email');	
		$client_id = $wpdb->get_var($wpdb->prepare("SELECT client_id From   " . $wpdb->prefix . sm_bookings . " WHERE id = %d",$edtid));		
		$service_id = $wpdb->get_var($wpdb->prepare("SELECT service_id From   " . $wpdb->prefix . sm_bookings . " WHERE id = %d",$edtid));
		$empid=$wpdb->get_var($wpdb->prepare("SELECT emp_id From  " . $wpdb->prefix . sm_bookings . " WHERE id = %d",$edtid));
        $emp_nam=$wpdb->get_var($wpdb->prepare("SELECT emp_name From  " . $wpdb->prefix . sm_employees . " WHERE id = %d", $empid)); 
		$day = $wpdb->get_var($wpdb->prepare("SELECT day From   " . $wpdb->prefix . sm_bookings . " WHERE id = %d",$edtid));				
		...
		$app_cancel = $wpdb->get_var($wpdb->prepare('SELECT app_cenceled FROM ' . $wpdb->prefix . sm_customer_notifications));
		if($app_cancel==1)
		{
	...
	if($status=="Approval Pending" && $emi==1)
	{
		...
		$empid=intval($_REQUEST['empid']);
			...
			$booked_service_name = $wpdb->get_var($wpdb->prepare('SELECT name FROM ' . $table_name . ' WHERE id = %d',$serid));
			$date = intval($_REQUEST['day'])."/".intval($_REQUEST['month'])."/".intval($_REQUEST['year']);
			$table_name = $wpdb->prefix . "sm_emails";
			$message_stored = $wpdb->get_var($wpdb->prepare('SELECT content  FROM ' . $table_name . ' WHERE type = "single_client"'));
			$subject_stored = $wpdb->get_var($wpdb->prepare('SELECT subject  FROM ' . $table_name . ' WHERE type = "single_client"'));
			$table_name = $wpdb->prefix . "sm_email_signature";
			$em_signature= $wpdb->get_var($wpdb->prepare('SELECT initial  FROM ' . $table_name));
			$title=get_bloginfo('name');
			$table_name = $wpdb->prefix . "sm_employees";
			$emp_nam = $wpdb->get_var($wpdb->prepare('SELECT emp_name  FROM ' . $table_name . ' WHERE id = %d', $empid));
			$name = $wpdb->get_var($wpdb->prepare("SELECT name From  " . $wpdb->prefix . sm_clients . " WHERE id = %d", $client_id));        
			$email = $wpdb->get_var($wpdb->prepare("SELECT email From  " . $wpdb->prefix . sm_clients . " WHERE id = %d", $client_id));	     
			$telephone = $wpdb->get_var($wpdb->prepare("SELECT mobile From  " . $wpdb->prefix . sm_clients . " WHERE id = %d", $client_id));
			$lastnam = $wpdb->get_var($wpdb->prepare("SELECT lastname From  " . $wpdb->prefix . sm_clients . " WHERE id = %d", $client_id));
			...
			Booking Service Provider: $emp_nam";	
				...
				$customer_app_booked = $wpdb->get_var($wpdb->prepare('SELECT app_booked FROM ' . $table_name));
				if($customer_app_booked==1)
				{
					wp_mail($to,$subject_stored,$message_final,$headers);
				}
				$table_name = $wpdb->prefix . "sm_emails";
				$msg_stored = $wpdb->get_var($wpdb->prepare('SELECT content  FROM ' . $table_name . ' WHERE type = "admin"'));
				$msg_sub = $wpdb->get_var($wpdb->prepare('SELECT subject FROM ' . $table_name . ' WHERE type = "admin"'));
				...
				$b_id = $wpdb->get_var($wpdb->prepare('SELECT id FROM ' . $table_name . ' where id = %d', intval($_REQUEST['editid'])));
				
			
				
				$approve = "<a href=\"".admin_url()."admin-ajax.php?action=confirm&actionappbok=approve&id=".$b_id."\">Approve Booking</a>";
				$msg_5 = str_replace("[approve]", $approve, $msg_4);
				$disapprove = "<a href=\"".admin_url()."admin-ajax.php?action=confirm_disapprove&booking=".$b_id."\">Disapprove Booking</a>";
				...
				$staff_app_booked = $wpdb->get_var($wpdb->prepare('SELECT app_booked FROM ' . $table_name));
				wp_mail($admin_email, $msg_sub, $msg_8, $headers);
			}
}
die;
}
