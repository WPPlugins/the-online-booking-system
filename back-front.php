...
<?php
	...
	else if($_REQUEST['method'] == "checkemaill")
	{
		...
		$ckemail  =  $wpdb->get_var(
			$wpdb->prepare(
				'SELECT count(id)  FROM ' . $wpdb->prefix . sm_clients . ' where email = %s',
				$email
			)
		);
		if($ckemail==0)
		...
		else
		{
			$lname  =  $wpdb->get_var(
				$wpdb->prepare(
					'SELECT lastname  FROM ' . $wpdb->prefix . sm_clients . ' where email = %s',
					$email
				)
			);
			$mobile  =  $wpdb->get_var(
				$wpdb->prepare(
					'SELECT mobile  FROM ' . $wpdb->prefix . sm_clients . ' where email = %s',
					$email
				)
			);
			$telephone  =  $wpdb->get_var(
				$wpdb->prepare(
					'SELECT telephone  FROM ' . $wpdb->prefix . sm_clients . ' where email = %s',
					$email
				)
			);
			$addressLine1  =  $wpdb->get_var(
				$wpdb->prepare(
					'SELECT addressLine1  FROM ' . $wpdb->prefix . sm_clients . ' where email = %s',
					$email
				)
			);
			$addressLine2  =  $wpdb->get_var(
				$wpdb->prepare(
					'SELECT addressLine2  FROM ' . $wpdb->prefix . sm_clients . ' where email = %s',
					$email
				)
			);
			$city  =  $wpdb->get_var(
				$wpdb->prepare(
					'SELECT city  FROM ' . $wpdb->prefix . sm_clients . ' where email = %s',
					$email
				)
			);
			$postalcode  =  $wpdb->get_var(
				$wpdb->prepare(
					'SELECT postalcode  FROM ' . $wpdb->prefix . sm_clients . ' where email = %s',
					$email
				)
			);
			$country  =  $wpdb->get_var(
				$wpdb->prepare(
					'SELECT country  FROM ' . $wpdb->prefix . sm_clients . ' where email = %s',
					$email
				)
			);
			if($lname=="")
									...
									<table class="activity_datatable" width="100%" border="0" cellspacing="0" cellpadding="8">
										<tbody>
											...
											<?php
														...
														$cur_icon =  $wpdb->get_var($wpdb->prepare('SELECT currency_sign  FROM ' . $wpdb->prefix . sm_currency . ' where currency_used = %s', "1"));
														for( $i = 0; $i < $count; $i++)
														{
														$idd=$hour[$i]->id;
														$nam=$hour[$i]->name;
