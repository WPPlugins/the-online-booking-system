...
function sendmail_callback(){
	...
	$admin_email = get_settings('admin_email');
		...
		if($emial == 1)
		{
			...
			$hour = $time[0];
			$minute = $time[1];
			$emp_nam=$wpdb->get_var($wpdb->prepare("SELECT emp_name From  " . $wpdb->prefix . sm_employees . " WHERE id = %d", intval($_REQUEST['empnam']))); 
			$emp_email=$wpdb->get_var($wpdb->prepare("SELECT email From  " . $wpdb->prefix . sm_employees . " WHERE id = %d", intval($_REQUEST['empnam']))); 
			$lastnam = $wpdb->get_var($wpdb->prepare("SELECT lastname From  " . $wpdb->prefix . sm_clients . " WHERE id = %d", $existing_client_id)); 		 
			$name = $wpdb->get_var($wpdb->prepare("SELECT name From  " . $wpdb->prefix . sm_clients . "  WHERE id = %d", $existing_client_id));        
			$email = $wpdb->get_var($wpdb->prepare("SELECT email From   " . $wpdb->prefix . sm_clients . "  WHERE id = %d", $existing_client_id));	      
			$telephone = $wpdb->get_var($wpdb->prepare("SELECT mobile From   " . $wpdb->prefix . sm_clients . " WHERE id = %d", $existing_client_id));
			$ser_name = $wpdb->get_var($wpdb->prepare("SELECT name From  " . $wpdb->prefix . sm_services . "  WHERE id = %d", intval($_REQUEST['bservice'])));		 
