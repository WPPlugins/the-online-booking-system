...
{
	if($_REQUEST['method'] == 'AddCurrency')
	{
	...
	else if($_REQUEST['method'] == 'UpdateCurriences')
	{
			$id = intval($_POST['id']);
	...
	else if($_REQUEST['method'] == 'deleteCurrency')
	{
	...
	else if($_REQUEST['method'] == 'saveSettings')
	{
				...
				$qry10 = $wpdb->prepare("UPDATE $table_name set  book_header = %s, thanks = %s where id = '1'", $bhedr, $thankhdr);
				$wpdb->query($qry10);
		}
}
else
	...
	<script type="text/javascript">
		...
		function funcurrency()
		{
					jQuery(document).ready(function($) 
					{
							...
							var result = jQuery.ajax({
										type: "POST",
										data: "currency="+currency+"&bhedr="+bheadr+"&thankshd="+thankshd+"&method=saveSettings",
										url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=setting",
		...
		function add_currency()
		{
			...
			{	
						...
						({
							type: "POST",
							data: "nam="+nam+"&sym="+syb+"&method=AddCurrency",
							url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=setting",
							success: function(data) 
							{
								...
								({
										...
										url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=settingrebind",								
										success: function(data)
										{	
												var temp=data;
		...
		function edit_mode(elm)
		{
			jQuery(document).ready(function($) 
			{
				...
				document.getElementById('save_'+elm.name).style.display='inline-block';
				document.getElementById('cancelc_'+elm.name).style.display='inline-block';
		...
		function save_currency(el)
		{
			...
			{	
				...
				if(currency_name_value!="" && currency_sign_value!="")
				{
					...
					({
					type: "POST",
					data: "currency_name="+currency_name_value+"&currency_sign="+currency_sign_value+"&id="+id+"&method=UpdateCurriences",
					url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=setting",
					success: function(data) 
					{
		...
		function delete_currency(currency) 
		{
			...
			{   
				...
				{   
					...
					jQuery.ajax({
						type: "POST",
						data: "idd=" + currency + "&method=deleteCurrency",
						url : '<?php echo admin_url('admin-ajax.php'); ?>' + "?action=setting",
						success: function(data) 
						{
		...
		function cancel_save_currency(ec)
		{
				...
				document.getElementById('edit_'+ec.name).style.display='inline-block';
				document.getElementById('save_'+ec.name).style.display='none';
				document.getElementById(ec.name).style.display='inline-block';
		}
						...
						<ul class="form_fields_container">
							<li>
								...
								<div class="form_input">
									...
									$currency_sel = $wpdb -> get_var($wpdb->prepare('SELECT currency FROM ' . $wpdb -> prefix . sm_currency . ' where currency_used = 1'));
									...
									$bhdr = $wpdb -> get_var($wpdb->prepare('SELECT book_header   FROM ' . $wpdb -> prefix . sm_settings . ' where id = 1'));
									$thank = $wpdb -> get_var($wpdb->prepare('SELECT thanks   FROM ' . $wpdb -> prefix . sm_settings . ' where id = 1'));
									?>
									...
									<a style="margin: 2px 0px 2px 20px;text-shadow:none" class="greyishBtn button_small colorbox" href="#manageCurrency" ><span class="iconsweet">*</span>Manage Currencies</a>
									<a class="greyishBtn button_small colorbox"  href="#addcurr" style="text-shadow:none">Add Currency</a>
								</div>
							</li>
							<li>
							...
							<li>
								<a href="http://www.wp-online-booking-system.com" target="_blank"><img style="margin-left: -5px;" src="<?php echo $url;?>images/paypal_activate_limited.jpg" ></a>
							</li>
							<li>
								<label>Booking Form Header</label>
								<div class="form_input">
									<textarea id="elm1" name="elm1" rows="15" cols="100"  class="tinymce" style="width:800px !important;">
							...
							<li>
								...
								<div class="form_input">
									<textarea id="cust1" name="cust1" rows="15" cols="100"  class="tinymce" style="width:800px !important;">
...
<div style="width:500px;" id="manageCurrency">
...
<div  id="addcurr" style="width:600px;">
